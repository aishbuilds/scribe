class Request < ActiveRecord::Base
  ...
  def broadcast_requests
    ...
    sms_requests(user_details.map(&:phone_no).compact)
    email_requests(user_details.map(&:user))
  end

  def sms_requests(phone_numbers)
    sms_string = "Hi! #{self.user.user_detail.try(:name)} is in need of a scribe on #{self.start_time.to_date} at #{self.address}. We have emailed you the details!"
